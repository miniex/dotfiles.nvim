...
- **UI** — Catppuccin Mocha base, retoned to a two-color damin palette (`#98ABCC` cool blue / `#E890B0` warm pink) — all `blue/sapphire/sky/teal` collapse to the blue, all `pink/flamingo/mauve/rosewater` collapse to the pink, mirroring [`fish-theme-damin`](https://github.com/miniex/fish-theme-damin)'s tone-on-tone identity. Custom lualine theme renders text-on-transparent (no filled mode badges); mode is signaled by the colour of the leading `✿` glyph (blue=normal, pink=insert/visual, red=replace), right end uses `❥ HH:MM`. neo-tree and bufferline match the same glyph set (`✿` modified, `✗` close/error, `✓` added, `?` untracked, `⇣` unstaged — all from the Dingbats block, no nerd font required for these specifically). Snacks dashboard header is a 5-step gradient between the two anchors. Plus bufferline (buffer mode, open-order sort, hides `[No Name]` and tabpage indicators) + smear-cursor (tuned to smooth-follow without a trail; the smear pulses only on terminal pane entry) + modicator + fidget
...
- **Theme** — `lua/plugins/ui/themes.lua`. Catppuccin Mocha base; the two damin anchor colors (`damin_blue` / `damin_pink`) at the top of the file are the only knobs you need to retone the whole UI. `color_overrides.mocha` maps the cool-blue family (`blue/sapphire/sky/teal`) and the warm-pink family (`pink/flamingo/mauve/rosewater`) onto those two anchors — change either anchor and every section that uses the family follows. `custom_highlights` pins `WinSeparator` / `VertSplit` / `FloatBorder` to the pink. Catppuccin compiles highlights to `~/.cache/nvim/catppuccin` automatically; no manual cache management.
...
| Theme tweaks not showing up | `rm -rf ~/.cache/nvim/catppuccin` and restart — catppuccin recompiles on next launch. If the bar still looks wrong, check that `lualine.lua` reloaded too (`:Lazy reload lualine.nvim`) |
