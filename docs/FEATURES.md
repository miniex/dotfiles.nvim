...
- **Files** — Neo-tree (floating). `<leader>e` toggle, `<leader>o` reveal current file. Directory rows show recursive total size instead of the default `-` — computed async, so rows fill in progressively without blocking; sizes use IEC binary units (KiB/MiB).
