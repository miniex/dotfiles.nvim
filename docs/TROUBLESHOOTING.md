...
| Missing prereq tools                              | `:checkhealth dotfiles` (in-editor: enabled langs → installed Mason servers, tools, clipboard) or `./tools/health.sh` (shell)                                          |
...
| Neotest discovery fails                           | adapter for that language is missing or its CLI isn't on `$PATH` (`pytest` / `go` / `mix` / `gtest` / `cargo` / `zig` / `vitest`)                                      |
