...
- `after/ftplugin/<ft>.lua` — buffer-local options Neovim auto-sources on `FileType` (after the built-in / plugin ftplugins, so it wins). Used for `go` / `make` (tabs, overriding the global `expandtab`), `gitcommit` / `markdown` (wrap, 72-col), `python` (venv keymap), and `yaml` / `json` (2-space). Add a file named after the filetype to set its own buffer options.
