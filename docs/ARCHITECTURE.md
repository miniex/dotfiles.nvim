...
| Launch modes            | `lua/config/globals.lua` `single_file` / `file_launch` / `multi_dir` | `nvim` (full IDE); `nvim <dir>` chdir's in (bare launch if inaccessible); `nvim <file>` (single, no session); multi-file (tabs, no session); `dir1 dir2` (per-dir tcd, manual session). Read by bufferline / persistence / autocmds. |
