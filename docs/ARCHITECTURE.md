...
| Launch modes            | `lua/config/globals.lua` `vim.g.single_file` / `file_launch` | `nvim`/`<dir>` (full IDE+session) vs `nvim <file>` (single, no session) vs multi-file; read by bufferline / persistence / autocmds.          |
